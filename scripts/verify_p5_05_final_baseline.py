...
def canonical_notebook_sha256(path):
    ...
    for cell in notebook.get("cells", []):
        ...
        source = "".join(str(x) for x in source_value) if isinstance(source_value, list) else str(source_value)
        badge_only = (
        ...
        if not badge_only:
            cells.append({"cell_type": cell.get("cell_type", ""), "source": source.rstrip()})
    payload = {
...
def main():
    ...
    Draft202012Validator(schema, format_checker=Draft202012Validator.FORMAT_CHECKER).validate(baseline)
    ...
    print(f"P6 post-freeze historical paths deferred: {p6_deferred}")
